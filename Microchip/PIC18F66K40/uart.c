...
{   //Set baud rate 115200: 64MHz/(4 * (138+1) = 115,107 (-0.08%)
    ...
    SP1BRGH = 0;                        //0: high byte = 0
    SP1BRGL = 138;                       //138: divide by 138+1
    BAUD1CONbits.CKTXP = 0;             //0: Idle TX state is high
    ...
    IPR3bits.RC1IP = 1;                 //1: High interrupt priority (RX)
    IPR3bits.TX1IP = 1;                 //1: High interrupt priority (TX)
    PIE3bits.RC1IE = 1;                 //1: Enable interrupts
    
    //Pin Peripheral selection
    RX1PPS = 0b00010110;    //C6
    RC7PPS = 0xC;    //EUSART1 TX
...
UINT8 SendUART( char *str, UINT8 nBytes )
{
    ...
    gieh = GIEH;
    GIEH = 0;               //CRITICAL SECTION: DISABLE INTERRUPTS
    send.nB += nBytes;      //Update buffer count
    GIEH = gieh;               //CRITICAL SECTION: DISABLE INTERRUPTS
